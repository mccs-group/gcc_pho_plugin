    ...
    for (int i = 0; i < plugin_info->argc; i++) {
        ...
        if (!strcmp(plugin_info->argv[i].key, "help")) {
            fprintf(
                ...
                "executed "
                "passes\n"
                "\t\tsymbols : prints to stdout symbols that are "
                "optimized (after all IPA passes)\n\n"
                ...
                "file "
                "name\n"
                "\tdyn_replace - set plugin into per-function pass replace "
                "mode (communication is done via unix sockets):\n"
                "\t\tlearning - optional argument; if specified sets order of "
                "sends\n"
                "\t\tand receives that can be used for model training");
        }
    }
    return 0;
}
